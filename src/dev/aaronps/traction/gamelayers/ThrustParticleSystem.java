...
public class ThrustParticleSystem implements ParticleSystem, GameLayer
{
    ...
    private static final double outangle                        = Math.PI
                                                                        / 8
                                                                        - Math.PI
                                                                        / 16;
    ...
    public ThrustParticleSystem()
    {
        ...
        for ( int n = 0; n < MAX_DOUBLES; n++) rnd_double[n] = rnd.nextDouble();
        for ( int n = 0; n < MAX_FLOATS; n++) rnd_float[n] = rnd.nextFloat();
        rnd = null;
    ...
    public void logic( float time)
    {
        if (active)
        {
            ...
            final int to_generate = Math.round( PARTICLES_PER_THRUST_PER_SECOND
                    * time );
